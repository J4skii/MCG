        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-8">
          {STATS.map((stat, index) => (
            <div key={index} className="flex flex-col items-center md:items-start text-center md:text-left">
              <div className="flex items-baseline text-4xl font-bold text-slate-900 mb-2">
                {stat.prefix && <span className="text-gray-500 mr-1">{stat.prefix}</span>}
                <AnimatedCounter value={stat.value} />
                {stat.suffix && <span className="text-blue-600 ml-1">{stat.suffix}</span>}
              </div>
