  ...
  if (!user) {
    return (
      <div className="min-h-[70vh] flex flex-col items-center justify-center text-center px-4">
        ...
        <h2 className="text-2xl font-bold text-slate-900 mb-2">No active session found</h2>
        <p className="text-gray-500 mb-8">Please register or log in to view your dashboard.</p>
        <button 
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="mb-10 flex flex-col md:flex-row md:items-start justify-between gap-4">
            <div className="flex-grow">
                {isEditing ? (
                   <div className="bg-white p-6 rounded-xl border border-blue-200 shadow-md max-w-2xl">
                       <h3 className="font-bold text-lg mb-4 text-slate-900">Edit Profile</h3>
                       <div className="grid md:grid-cols-2 gap-4">
                          ...
                          {user.type === 'entrepreneur' && (
                            <div>
                               <label className="block text-xs font-semibold text-gray-500 mb-1">Reg Number</label>
                               <input 
                                 type="text" 
                                 ...
                                 className="w-full px-3 py-2 border rounded-lg text-sm"
                               />
                               {editErrors.regNumber && <p className="text-red-500 text-xs mt-1">{editErrors.regNumber}</p>}
                            </div>
                          )}
                       </div>
                       <div className="flex justify-end gap-3 mt-4">
                           <button onClick={handleCancelEdit} className="flex items-center gap-1 text-sm text-gray-500 hover:text-gray-700 px-3 py-2">
                             <X size={16} /> Cancel
                           </button>
                           <button onClick={handleSaveProfile} className="flex items-center gap-1 text-sm bg-blue-600 text-white px-4 py-2 rounded-lg hover:bg-blue-700">
                  ...
                  <div className="group relative">
                      ...
                      <p className="text-gray-600 flex items-center gap-2">
                         {user.companyName} | <span className="uppercase font-semibold text-blue-600 text-xs bg-blue-50 px-2 py-0.5 rounded">{user.type}</span>
                      </p>
                      {user.phone && <p className="text-sm text-gray-500 mt-1">{user.phone}</p>}
                      {user.regNumber && <p className="text-xs text-gray-400 mt-0.5">{user.regNumber}</p>}
                  </div>
                )}
            </div>
            ...
            <div className="flex items-center gap-3 bg-white px-4 py-2 rounded-lg border border-gray-200 shadow-sm self-start">
                ...
                <span className="font-medium text-sm text-gray-700 capitalize">Status: {user.status}</span>
            </div>
        </div>
            ...
            <div className="lg:col-span-2 space-y-8">
                <div className="bg-white rounded-xl p-8 border border-gray-200 shadow-sm">
                    <div className="flex justify-between items-center mb-6">
                        ...
                        <span className="text-xs font-semibold text-gray-400 uppercase tracking-wider">Step {currentStepIdx + 1} of {flowSteps.length}</span>
                    </div>
                    
                    {/* Dynamic Stepper */}
                    <div className="relative pt-4 pb-12 px-2">
                        {/* Connecting Line */}
                        <div className="absolute top-7 left-0 w-full h-1 bg-gray-100 rounded-full -z-10"></div>
                        <div 
                           className="absolute top-7 left-0 h-1 bg-blue-600 rounded-full -z-10 transition-all duration-700"
                           ...
                           {flowSteps.map((step, idx) => {
                               ...
                               return (
                                   <div key={step.id} className="flex flex-col items-center relative group">
                                       ...
                                       <div className={`absolute top-8 w-24 text-center text-[10px] sm:text-xs font-medium transition-colors ${isCurrent ? 'text-blue-700 font-bold' : 'text-gray-400'}`}>
                                           {step.title}
                                       </div>
                                   </div>
                               );
                    ...
                    <div className="bg-blue-50 border border-blue-100 rounded-lg p-5 flex gap-4 items-start">
                        ...
                        <div>
                            <h4 className="font-bold text-blue-900 text-sm uppercase tracking-wide mb-1">Current Step: {currentStepData.title}</h4>
                ...
                <div className="grid md:grid-cols-2 gap-6">
                    <div className="bg-white p-6 rounded-xl border border-gray-200 shadow-sm">
                        <FileText className="text-gray-400 mb-4" />
                        <h4 className="font-bold text-slate-900">Submitted Documents</h4>
                     ...
                     <div className="bg-white p-6 rounded-xl border border-gray-200 shadow-sm opacity-50 relative overflow-hidden">
                        <div className="absolute inset-0 flex items-center justify-center bg-gray-50/50 backdrop-blur-[1px]">
                             <span className="bg-white px-3 py-1 rounded-full shadow-sm text-xs font-bold text-gray-500 flex items-center gap-1">
                                <ShieldCheck size={12} /> Locked
                             </span>
                        </div>
                        <ShieldCheck className="text-gray-400 mb-4" />
                        <h4 className="font-bold text-slate-900">Deal Room Access</h4>
                        <p className="text-sm text-gray-500 mt-2">Available once {user.type === 'entrepreneur' ? 'packaging' : 'verification'} is complete.</p>
                    </div>
                </div>
            </div>
                ...
                <div className="bg-slate-900 text-white p-6 rounded-xl">
                 ...
                 <div className="bg-white p-6 rounded-xl border border-gray-200 shadow-sm">
                    ...
                    <ul className="space-y-4">
                        <li className="text-sm border-b border-gray-50 pb-3">
                            <div className="font-medium text-slate-800">Registration Complete</div>
                            <div className="text-xs text-gray-500">{new Date(user.registrationDate).toLocaleDateString()}</div>
                        </li>
                         <li className="text-sm">
                            <div className="font-medium text-slate-800">Documents Uploaded</div>
                            <div className="text-xs text-gray-500">{new Date(user.registrationDate).toLocaleDateString()}</div>
                        </li>
                    </ul>
                </div>
